...
import { getAllDepartureAdminService, getAllDestinationAdminService } from '../../services/TripService';


const HomePage = () => {
    const [departureList, setDepartuteList] = useState({})
    const [destinationList, setDetinationList] = useState({})
    ...
    const getAllDeparture = async () => {
        ...
        if(res && res.status === 'OK') {
            setDepartuteList(res.data)
        }
    }

    const getAllDetination = async () => {
        let res = await getAllDestinationAdminService()
        if(res && res.status === 'OK') {
            setDetinationList(res.data)
        }
    }
    return (
